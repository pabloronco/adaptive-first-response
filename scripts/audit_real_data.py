...
def _key_cama(row: dict[str, str]) -> tuple[str, str, str]:
    return (_norm(row.get("SiteID")), _norm(row.get("Year")), _norm(row.get("Month")))
...
def _key_coord(row: dict[str, str]) -> tuple[str, str]:
    return (_norm(row.get("SiteID")), _norm(row.get("Year")))
...
def audit(
    ...
    matched_coords = [
        key for key in cama_keys if (key[0], key[1]) in coord_by_key
    ]
    ...
    years = sorted({_norm(row.get("Year")) for row in cama_rows if _norm(row.get("Year"))})
    sites = sorted({_norm(row.get("SiteID")) for row in cama_rows if _norm(row.get("SiteID"))})
...
def _print_report(report: dict[str, Any]) -> None:
    ...
    print(
        ...
        f"zero fraction={_pct(1.0 - cama['detection_row_fraction']) if cama['detection_row_fraction'] is not None else 'n/a'}"
    )
    print(
        f"Exact effort join (SiteID,Year,Month): {effort['matched_rows']}/{cama['rows']} "
        f"({_pct(effort['coverage_fraction'])})"
    )
    print(
        f"Coordinate join (SiteID,Year): {coords['matched_rows']}/{cama['rows']} "
        f"({_pct(coords['coverage_fraction'])})"
    )
